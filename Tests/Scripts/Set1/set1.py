...
# ---------------- ARGUMENT PARSER ----------------
parser = argparse.ArgumentParser(description="Analyze test results from a folder")
parser.add_argument(
    "--dir",
    type=str,
    default=".",
...
tests = sorted(
    ...
    key=lambda d: d["environment"]["NUMBER_OF_WORKER_PODS"]
)

# ---------------- PROCESS DATA ----------------
var_blocks = [
    {
        "K": d["environment"]["NUMBER_OF_WORKER_PODS"],
        "ins": [
            {
                "length": float(key),
...
for met in var_blocks:
    ...
    plt.plot(x, y, "o")
    plt.plot(ax, fy, "-", label="W=" + str(met["K"]))
    plt.legend(loc="upper left")

# Save inside the same directory
output_path = os.path.join(DATA_DIR, "set1.png")
